...
from nomz.models import Restaurant, User
...
def check_rest2_owner():
    ...
    try:
        ...
        if hasattr(user, 'restaurant_profile'):
            ...
            print(f"Restaurant Profile found: ID={res.id}, Name='{res.name}', DisplayName='{res.display_name}'")
        else:
            ...
            if res:
                print(f"Restaurant linked by owner_id found: ID={res.id}, Name='{res.name}'")
            else:
