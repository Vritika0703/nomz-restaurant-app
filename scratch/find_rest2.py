...
from nomz.models import Restaurant, RestaurantSearch, RestaurantSourceRecord, User
...
def find_rest2():
    ...
    print(f"User (username): {u.count()} found. {list(u.values_list('username', flat=True))}")

    # Search Restaurant
    res1 = Restaurant.objects.filter(name__icontains='rest2')
    print(f"Restaurant (name): {res1.count()} found. {list(res1.values_list('name', flat=True))}")
    
    res2 = Restaurant.objects.filter(display_name__icontains='rest2')
    print(f"Restaurant (display_name): {res2.count()} found. {list(res2.values_list('display_name', flat=True))}")

    # Search RestaurantSearch
    try:
        res3 = RestaurantSearch.objects.filter(name__icontains='rest2')
        print(f"RestaurantSearch (name): {res3.count()} found. {list(res3.values_list('name', flat=True))}")
